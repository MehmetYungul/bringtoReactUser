...
import useGetCategoriesForStore
  from "../../../api-manage/hooks/react-query/categories-details/useCategoriesDetailsForStore";
...
} from "../../../api-manage/hooks/react-query/product-details/useGetDiscountedItems";
...
const SearchResult = (props) => {
    const {searchValue, configData, isSearch, fromAllCategories} = props;
    ...
    const [selectedCategories, setSelectedCategories] = useState(
        searchValue !== "category" || isSearch === "true" ? [] : [id]
    );
    ...
    useEffect(() => {
        if (category_id) {
            if (currentTab === 0) {
                if (
                    selectedCategories?.length > 0 && isSmall
                        ? offset === 0 || offset === 1
                        : offset === 1
                ) {
                    refetchCategoryData();
    ...
    const handleAPiCallOnSuccess = (res) => {
        ...
        const initialHigh2LowSortedData =
        ...
        if (currentTab === 0) {
            ...
            const newArray = isSmall
                ? offset > 0
                    ? [...pageData, ...initialHigh2LowSortedData]
                    : initialHigh2LowSortedData
                : offset > 1
                    ? [...pageData, ...initialHigh2LowSortedData]
                    : initialHigh2LowSortedData;
            console.log({initialHigh2LowSortedData});
            ...
            // else {
            //   setPageData(initialHigh2LowSortedData);
            // }
        } else {
            setTotalDataCount(res?.total_size);
            const newArray =
    ...
    useEffect(() => {
        if (qureyValue && fromAllCategories !== "allCategories") {
            setOffset(0);
            serachRefetch();
        }
    }, [qureyValue, currentTab]);
    useEffect(() => {
        if (currentTab === 0) {
    ...
    const getPageData = () => {
        const categoryWiseFiltered = qureyValue
            ? getCategoriesWiseFilter()
            : pageData;
        const isChecked =
            filterData?.length > 0
    ...
    const renderData =
        categoryData || storeData || allItems || specialOfferData || searchData;
    ...
    const isApiLoading = isLoadingAllStores || allItemsLoading;
    return (
        <CustomContainer>
            <CustomStackFullWidth alignItems="center" justifyContent="center">
                ...
                <CustomBoxFullWidth>
                    <SideBarWithData
                        ...
                        isFetchingSearchAPi={isFetchingSearchAPi}
                        isLoadingSearch={isLoadingSearch}
                        specialOfferLoading={specialOfferLoading}
                        isInitialRefetching={
                            isRefetchingSearch ||
                            isCategoryRefetching ||
                            isRefetchingCategoriesStores
